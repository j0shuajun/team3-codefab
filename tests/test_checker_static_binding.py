...
class TestExecutorUsesStaticBindingWhenAvailable:
    def test_reading_local_variable_uses_get_at_not_dynamic_get(self, monkeypatch):
        ...
        assert not any(call[0] == "dynamic_get" and call[1] == "a" for call in SpyEnvironment.calls)

    def test_assigning_local_variable_uses_assign_at_not_dynamic_assign(self, monkeypatch):
        statements = resolve([
            BlockStmt([
                VarStmt(token("a"), LiteralExpr(1)),
                BlockStmt([
        ...
        assert not any(
            call[0] == "dynamic_assign" and call[1] == "a" for call in SpyEnvironment.calls
        )
