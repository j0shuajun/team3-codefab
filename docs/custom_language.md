...
한글 keyword, `ctrl c` 기능이 후보로 나왔고, 현재 방향은 `ctrl_c()`를 CodeFab 코드 안에서 호출하는
언어 기능으로 구현하는 것이다. `explain` 기능과 한글 키워드도 함께 구현 중이다.
...
- CodeFab 코드 안에서 `ctrl_c()`를 호출해 추천/보조 기능을 사용할 수 있게 하는 방향으로 확장 중이다.
- `explain` 기능으로 코드나 실행 흐름을 사람이 이해하기 쉽게 풀어주는 방향을 구현 중이다.
- 한글 키워드로 더 친숙하게 코드를 작성할 수 있도록 구현 중이다.
...
Issue #12의 `ctrl c 기능` 아이디어는 shell 명령이 아니라 CodeFab 언어 안에서 호출하는 `ctrl_c()` 함수로
구현하기로 했다. 이 방향은 "자주 쓰는 코드나 다음에 실행할 코드를 추천받는다"는 사용 경험을 prompt shell
외부에서도 설명할 수 있게 만든다.
...
- `ctrl_c()`가 현재 실행 맥락이나 history를 바탕으로 도움말, 추천, 재사용 후보를 제공한다.
...
| 기능 | 의도 | 예시 방향 |
|---|---|---|
| `ctrl_c()` | CodeFab 코드 안에서 추천/보조 기능 호출 | `ctrl_c();` |
