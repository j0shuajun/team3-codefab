...
| 특징 | 설명 |
|---|---|
| `ctrl_c()` 추천 | CodeFab 코드 안에서 추천/보조 기능을 호출하는 방향으로 구현 중이다 |
| `explain` | 코드나 실행 흐름을 사람이 읽기 쉽게 설명하는 방향으로 구현 중이다 |
| 한글 키워드 | 영어 키워드가 낯선 사용자도 읽기 쉽게 작성하는 방향으로 구현 중이다 |
...
| 언어를 써보기 | `docs/custom_language.md` |
